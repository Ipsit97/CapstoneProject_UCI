...
import { NavLink, json, useNavigate, Link } from 'react-router-dom';
...
const CsvData = () => {
    ...
    const CommentsView = async (e) => {
        ...
        if (input === "comments") {
            try {
                const response = await axios.get(`http://backend-image:8000/result/${file_id}`);
            ...
            try {
                const response = await axios.get(`http://backend-image:8000/result/${file2_id}`);
                setIsUploadView(response.data.csv_data)
            } catch (error) {
                alert("Your file is deleted, Please Upload again!")
            }
    ...
    const handleBack = async () => {
        ...
        const response = await axios.post('http://backend-image:8000/remove-file', formData, {
            headers: {
    ...
    const sendMailHandler = async () => {
        ...
        const response = await axios.post('http://backend-image:8000/send_mail', formData, {
            headers: {
                'Content-Type': 'application/json'
            }
        });
            ...
            <div className="container bg-transparent mt-5" style={{ display: 'block' }}>
                <div className="d-flex justify-content-between mb-5 align-items-center">
                    ...
                    {loading ? (
                        ...
                        <button className="btn btn-primary" onClick={sendMailHandler}>Train Model</button>
                    )}
                </div>
