...
def process():
    ...
    if file:
        ...
        file_extentions = ".csv"
        file_id = dh.add_csv_file(email, file, file_extentions)
    ...
    if file2:
        ...
        file_extentions = ".csv"
        file2_id = dh.add_csv_file(email, file2, file_extentions)

    return jsonify({"message": "Files uploaded successfully", "file_id": file_id, "file2_id": file2_id, "email": email}), 200
...
def display_result(file_id):
    ...
    return jsonify({"csv_data": data, "csv_list_data": csv_data})
...
def remove_csv_files():
    ...
    files_id = [file_id, file2_id]
    for file1 in files_id:
        dh.remove_csv_file(email_id=email, csv_id=file1)
...
def send_mail_function():
    ...
    long_running_task(email, file_id, file2_id)
...
@celery.task
def long_running_task(email, csv_id, csv_id2):
    """
    ...
    Args:
        ...
        csv_id2 (str): ID of the second CSV file.
    """
    file_path = os.path.join(BASE_PATH, email, csv_id + '.csv')
    file_path2 = os.path.join(BASE_PATH, email, csv_id2 + '.csv')
...
@app.route('/get_file_path', methods=['GET'])
def get_file_path():
